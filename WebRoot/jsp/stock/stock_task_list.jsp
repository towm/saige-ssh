									...
									<tbody>
										<c:forEach items="${stasks}" var="stask">
											<tr>
												...
												<td data-original-title="task comment"
													data-content="${stask.message}" data-placement="top"
													data-trigger="hover"
													class="btn btn-default btn-sm popovers"><c:out
														value="${stask.name}" /></td>
												<td><c:out value="${stask.type}" /></td>

												<td data-original-title="product info"
													data-content="id: ${stask.product.id} Capacity: ${stask.product.id}"
													data-placement="top" data-trigger="hover"
													class="btn btn-default btn-sm popovers"><c:out
														value="${stask.product.name}" /></td>

												<td><c:out value="${stask.date}" /></td>
												<td><a href="subTaskList.do?listid=${stask.listid}"><c:out
															value="${stask.listid}" /></a></td>
												<td><c:out value="${stask.quality}" /></td>
												<td class="dropdown"><a href="javascript:;"
													class="dropdown-toggle" data-toggle="dropdown"><c:out
															value="${stask.progress}" /><b class="caret"></b></a>
													<ul class="dropdown-menu">
														<li><a href="updateStatus.do?id=${stask.id}&status=0">Set
																Not Started</a></li>
														<li><a href="updateStatus.do?id=${stask.id}&status=1">Set
																In Progress 20%</a></li>
														<li><a href="updateStatus.do?id=${stask.id}&status=2">Set
																In Progress 50%</a></li>
														<li><a href="updateStatus.do?id=${stask.id}&status=3">Set
																In Progress 80%</a></li>
														<li><a href="updateStatus.do?id=${stask.id}&status=4">Set
																Completed</a></li>
													</ul></td>

												<td><span class="badge bg-${stask.priority}"><c:out
															value="${stask.priority}" /></span></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
